...
function page_del() {
      ...
      var token = localStorage.getItem('token');
			$.ajax({
        //	url:"../queryLevel/index.php",
        //url:'http://tg.7900005.com/api/v1/preferential/goldHousekeeper/'+user+'/',
				...
				/*success:function (e) {
					if (user== "") {
						...
						if(e.status==-1){
                            ...
                            page();
                            $('.code').html(user);
                            $('.code1').html(parseFloat(dml_if(e.hdata)));
                            $('.code2').html(parseFloat(dml_if(e.ndata)));
                            $('.code3').html(e.hlevel);
                            $('.code4').html(e.nlevel);
                            $('.code5').html(e.needup);
                            $('.code6').html(e.wmoney);
