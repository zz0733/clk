  ...
  <div class="apply_open">
    ...
    <div class="content" v-if="spanActive ==0">
      ...
      <div class="caipiaoContent">
        <div class="bar" v-for="(item,i) in rebatedata" :key="i" :class="{refundList:caipiaoNum ==i}"
        ...
        <div class="setRefund">
          ...
          <div class="setRefund-main" @click="MaxRefund2">
            ...
            <div class="refund-row">
              <label>统一设置：</label>
              <img src="/static/public/image/userImg/jianshao.png" alt="" class="leftImg" @click="reduce2">
              <img src="/static/public/image/userImg/zengjia.png" alt="" class="rightImg" @click="increase2">
            </div>
    ...
    <div class="content" v-if="spanActive ==1">
      ...
      <!-- 反水设置 -->
      <div class="refundContent" style="top:-22px">
    ...
    methods: {
      ...
      submitOpen () {
        ...
        if (this.spanActive == 0) {
          ...
          let ifUserName = this.validateAccount(this.param.userName)
          if (!ifUserName) {
            this.toastText = '5~20位字母与数字用户名';
            (this.toastNum = 80), (this.toastShow = true)
            return false
          }
        ...
        this.$http.get(`${this.$HOST_NAME}/lottery/getrebatedata?type=member&id=${this.$store.state.mainState.userinfo.uid}`).then(res => {
          if (res && res.code == 200) {
            ...
            for (let key in res.data) {
              ...
              res.data[key].forEach((a, i) => {
                // a.rebate = this.toFloat(a.rebate);
                a.option = [{
                  value: '0.0',
                  name: '0%'
                }]
                for (let j = 1; j <= a.rebate * 10; j++) {
                  a.option.push({
                    value: String(j / 10).indexOf('.') != -1 ? j / 10 + '0' : j / 10 + '.00',
                    name: j / 10 + '%'
                  })
                }
              })
            ...
            // localStorage.setItem("rebatedata", JSON.stringify(arr))
            this.rebatedata = arr
      ...
      reduce2 () {
        this.caipiaoList.forEach(v => {
          v.rebate =
            v.rebate * 10 - 1 <= 0 ? '0.0' : String((v.rebate * 10 - 1) / 10).indexOf('.') != -1 ? String((v.rebate * 10 - 1) / 10) + '0' : String((v.rebate * 10 - 1) / 10) + '.00'
        })
      },
      increase2 () {
        // let refund = JSON.parse(localStorage.getItem("rebatedata"));
