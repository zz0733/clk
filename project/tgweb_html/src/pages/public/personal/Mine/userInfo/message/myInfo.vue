  ...
  <div class="myinfo">
    ...
    <div id="mescroll" class="mescroll" :class="{mainNone:detailShow ==false}">
      <div class="main" id="dataList" v-if="detailShow">
        <div class="row" @click="getDetail(item.id)" v-for="(item,i) in messageData" :key="i">
    ...
    <div class="info-detail" v-if="!detailShow">
      <!-- 信息详情头部 -->
      <div class="header">
        <span class="fl mh5-back" @click="detailShow =true"></span> {{detailData.title}}
      </div>
      <cube-scroll>
        <div class="main" v-html="detailData.content">
        </div>
  ...
  export default {
    ...
    methods: {
      getDetail (id) {
        this.$http
          ...
          .then(res => {
            ...
            this.detailData.send_time = this.$moment
              .unix(this.detailData.send_time - 0)
      ...
      getListDataFromNet (pageNum, successCallback, errorCallback) {
        ...
        setTimeout(() => {
          this.$http
            .post(`${this.$HOST_NAME}/member/message?page=${pageNum}`)
            .then(
              res => {
