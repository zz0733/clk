  ...
  <div class="result-outSide-wrap">
    <div v-if="trueAward.length!=3" class="wrap">
      <div class="lottery-result" :class="{'pk10Result':$route.meta.series=='pk10',}">
        <div class="left">
          <div style="text-align:left;"><span class="text-color">{{showLastIssue}}期</span></div>
          <div>开奖结果:</div>
        </div>
        <div class="right">
      ...
      <div class="container-lottery-timer">
        ...
        <i @click="$router.push({path:'/plays/trend',query:{id:$route.meta.id,series:$route.meta.series}})"
           class="icon iconfont h5-icon-H-qushi text-color"></i>
    ...
    <div v-if="trueAward.length==3" class="wrap-bot">
      <div class="lottery-result-bot">
        ...
        <div class="right-bot">
          ...
          <div class="ico">
            <span class="fengpan text-color" v-show="timer.djs_next_kaipan">已封盘</span>
            <label for="" v-show="!timer.djs_next_kaipan">
              <span class="jiezhi text-color">截止</span>
              <vp-timer class="ico-time" v-on:child-say="timeout" ref="timer"></vp-timer>
            </label>
            <i @click="$router.push({path:'/plays/trend',query:{id:$route.meta.id,series:$route.meta.series}})"
               class="icon iconfont h5-icon-H-qushi text-color"></i>
          </div>
