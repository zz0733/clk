<template>
  <div :style="{position:position }" class="betArea-decial-choose-cont">
    <div class="sel-box" v-if="$route.meta.series!='lhc'">
      <div class="left">
        <div>已选
          <span class="text-color">{{total.num}}</span> 注
        </div>
        <div>共
          <span class="text-color">¥ {{total.momey}}</span>
        </div>
      </div>
      <div class="right">
    ...
    <div class="betArea-decial-choose-money">
      <div class="input-wrap">
        ...
        <span class="input">
          <van-field type="tel" placeholder="输入金额" maxlength="10" matinput="" aria-invalid="false" @blur="inputBlur"
                     @keyup="calculate" v-model="total.inputVal"/>
          ...
          <span v-if="$route.meta.series=='lhc'" class="totle">共<label class="text-color">{{total.num}}</label>注</span>
        </span>
        <span @click="sure" class="sure bg-color ">立即投注</span>
      ...
      .input-wrap {
        ...
        .text {
          ...
          &::before {
            display: inline-block;
            content: "";
            background: url("../../../../../../assets/img/public/image/lottery/plays-tradition/del_icon@2x.png") no-repeat;
            width: 0.34rem;
            height: 0.34rem;
