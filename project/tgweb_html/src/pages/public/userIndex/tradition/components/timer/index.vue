<template>
  <div class="time-lottery-wrap">
    <div v-if="timer.timerModel!='days'" class="lottery-timer-center">
      ...
      <span class="timer text-color" v-show="timer.timeLeave>=3600">{{hoursDatas.hou>9?hoursDatas.hou:'0'+hoursDatas.hou}}</span>
      <span class="timer text-color" v-show="timer.timeLeave>=3600">:</span>
      <span class="timer text-color">{{hoursDatas.min>9?hoursDatas.min:'0'+hoursDatas.min}}</span>
      <span class="timer text-color">:</span>
      <span class="timer text-color">{{hoursDatas.sec>9?hoursDatas.sec:'0'+hoursDatas.sec}}</span>
    </div>
    <div v-if="timer.timerModel=='days'" class="lottery-timer-center">
      <span v-if="!this.timer.djs_next_kaipan"></span>
      <span class="tip" v-if="this.timer.djs_next_kaipan"></span>
      <span class="timer text-color">{{daysDatas.day>9?daysDatas.day:'0'+daysDatas.day}}</span>
      <span class="timer text-color doc active">天</span>
      <span class="timer text-color">{{daysDatas.hou>9?daysDatas.hou:'0'+daysDatas.hou}}</span>
      <span class="timer text-color doc active">时</span>
      <span class="timer text-color">{{daysDatas.min>9?daysDatas.min:'0'+daysDatas.min}}</span>
      <span class="timer text-color doc active">分</span>
    </div>
  </div>
