  ...
  <div class="home ">
    ...
    <div class="infoSetList">
      ...
      <div class="row">
        ...
        <div @click="goSetInfo('/set_info?msg=邮箱')" class="fr">
          <span> {{this.$store.state.mainState.userinfo.email?'-*****':'暂无设置'}}</span>
          <i class="fr cubeic-arrow"></i>
        </div>
      </div>
      <div class="row">
        ...
        <div @click="goSetInfo('/set_info?msg=微信')" class="fr">
          <span class="wechat-name">{{this.$store.state.mainState.userinfo.wechat?'-*****':'暂无设置'}}</span>
          <i class="fr cubeic-arrow"></i>
        </div>
      </div>
    </div>

    <div class="loginout bg-color " @click="exit">
    ...
    <van-actionsheet v-model="show" class="portraitImg">
      <div class="imgList clearfloat" ref="imgList" v-swipeleft="swipeleft" v-swiperight="swiperight">
        <img :src="item|capitalize" alt="" v-for="(item,i) in portraitList" :key="i" @click="toggleImg(i,item)"
             :class="{imgActive:imgActive===i}">
      </div>
      <div class="pulley">
        <span v-for="(item,i) in 3" :key="i" @click="toggle(i)" :class="{spanActive:spanActive===i}"></span>
      </div>
      <div class="notarize bg-color " @click="updataImg">
        确认
      </div>
    </van-actionsheet>
