  ...
  <div class="container">
    ...
    <div class="hall-out-wrap">
      ...
      <div class="all-container-wrap">
        <cube-slide ref="slide" :data="images" class="hall-swipe-wrap" :showDots="false" :autoPlay="true"
                    :interval="5000">
          <cube-slide-item v-for="(item, index) in images" :key="index">
            <a
              :href="item.targets?item.title.includes('download')?hallDatas.downLoadUrl:publicHost+'/'+item.targets:'#'">
              <img :src="item.img">
            </a>
          </cube-slide-item>
        </cube-slide>
      ...
      <div class="hall-content ">
        <div class="hall-content-top ">
          <van-notice-bar delay="0" :text="lantern"
                          :left-icon="require('@assets/img/public/image/common/home/message.png')"/>
          <div class="hall-content-type">
            <div class="type-list" :key="index" v-for="(item,index) in typelist" @click="showListFc(item)">
              <img :src='item.ico' alt="">
              <div :class="{'text-color':item.name==selName}" class="list-title">{{item.name}}</div>
            </div>
          </div>
        </div>
        <div class="hottab">
...
// ico图片获取
import mixin from "../game/pubilc.js";
