  ...
  <div class="footer">
    <ul>
      <li :class="{text:$route.path == item.path}" @click="toggle(item.path)" v-for="(item,i) in footerList" :key="i">
        ...
        <i :class="[item.class,{'text-color':$route.path == item.path}]"></i>
        <p :class="{'text-color':$route.path == item.path}">{{item.text}}</p>
      </li>
    </ul>
  </div>
</template>
        ...
        footerList: [
          {
            ...
            path: '/home',
            class: 'icon iconfont h5-icon-foot-home'
          },
          {
            text: '存款',
            ...
            class: 'icon iconfont h5-icon-foot-deposit'
          },
          {
            text: '客服',
            ...
            path: '/service',
            class: 'icon iconfont h5-icon-foot-customer'
          },
          {
            text: '投注记录',
            ...
            class: 'icon iconfont h5-icon-foot-compare'
          },
          {
            text: '我的',
            ...
            path: '/personal/mine',
            class: 'icon iconfont h5-icon-foot-my'
          }
        ]
      }
    },
      ...
      async getbannner () {
        ...
        if (res && res.code == 200) {
          window.location.href =
            window.location.origin + '/' + res.data[0].activityTargets
        }
      }
    },
    created () {
