  ...
  <div class="on-line" v-if="paymentData.length">
    ...
    <div v-if="$store.state.personal.itemDatas.classType!='qr_code'" class="content">
      ...
      <div class="content-main" v-if="spanActive ==i" v-show="!qrcodeShow" v-for="(item,i) in paymentData" :key="i">
        ...
        <div :class="{'active':!canClick}" class="submitPay" @click="onlinePayment(item.id,item.minAmount,item.maxAmount)" v-if="!qrcodeShow">
          确认提交
        </div>
...
export default {
  ...
  watch: {
    ...
    paymentData: function(newValue, oldValue) {
      ...
      if (
        this.$store.state.personal.itemDatas.classType == "qr_code" &&
        newValue[0]
      ) {
        this.url = this.publicUrl + newValue[0].qr_code;
        this.passKey.codeId = newValue[0].id;
        this.passKey.max_amount = newValue[0].max_amount;
        this.passKey.min_amount = newValue[0].min_amount;
      }

      if (this.$store.state.personal.navView == 4 && newValue) {
        this.bankList = newValue[0].bankCode;
      }
    }
  },
  filters: {
