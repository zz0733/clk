  ...
  <div class="content clearfloat">
    ...
    <div class="deposit-left fl">
      ...
      <div class="pay-bankinfo">
        ...
        <div class="row">
          <label>提款金额：</label>
          <input type="text" v-model="amount" :placeholder="'可提现金额'+availableAmount+'元'" onkeyup="value=value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')" @blur="hanlderAmount">
           <a href="javascript:;" @click="amount =availableAmount">全额提款</a>
        </div>
        ...
        <div class="row">
          <label>资金密码：</label>
          <input type="password" v-model="payPassword" maxlength="6">
        </div>
      </div>
    ...
    <div class="deposit-right fl">
      ...
      <Table :columns="columns" :data="data" no-data-text="<div style='line-height:453px;background:#f2f2f2;'><img src='/static/public/image/userImg/no-data.png' alt=''></div>"></Table>
    </div>
...
export default {
  ...
  methods: {
        ...
        if (res.code == 200) {
          ...
          if (!this.bankList.length) {
            if (localStorage.Public_User == 'test') {
              this.$error("试玩用户无权限，请注册正式用户", 8000);
            } else {
              this.$error("请绑定银行卡", 8000);
    ...
    application() {
      ...
      let isPwd = this.valiPwdAccount(this.payPassword);
      if (!isMoney) {
        this.toastShow = true;
        this.toastNum = 375;
