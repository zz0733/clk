  ...
  <div id="maincontainer">
    <div class="mainContent">
      ...
      <div class="mainbox">
        ...
        <div class="box">
          <ul id="slider">
            <li :class="[{'active':$route.query.id==item.id},item.class]" :key="index" v-for="(item,index) in rollList" @click="fetchgames(item)">
      ...
      <div ref="search-container" class="search-container">
        <div class="search-info">
            ...
            <div v-if="nowTypeList.type.list.length" class="search-detail">
              <div class="search-type">
                <dl class="search-game top">
                  <dt>游戏类型:</dt>
                  <dd>
                    <a href="javascript:;" @click="resulttype(item)" :key="index" :class="{'active':item.tag==currenttype}" v-for="(item,index) in  nowTypeList.type.list">{{item.name}}</a>
                  </dd>
                </dl>
                <dl v-if="nowTypeList.line.list.length" class="search-game bottom">
                  <dt>赔钱线数:</dt>
                  <dd>
                    <a href="javascript:;" @click="resultline(item)" :key="index" :class="{'active':item.tag==currentline}" v-for="(item,index) in nowTypeList.line.list">{{item.name}}</a>
                  </dd>
                </dl>
                <dl v-if="nowTypeList.thirt.list.length" class="search-game bottom">
                  <dt>游戏类型:</dt>
                  <dd>
                    <a href="javascript:;" @click="resultTag(item)" :key="index" :class="{'active':item.tag==currentTag}" v-for="(item,index) in nowTypeList.thirt.list">{{item.name}}</a>
                  </dd>
                </dl>
          ...
          <div ref="list_winner" :style="{height:offsetHeight+'px'}" v-show="$route.query.id!='42'&&$route.query.id!='31'&&$route.query.id!='29'&&$route.query.id!='27'" class="list_winner">
            <div>
          ...
          <div class="game-content">
            <ul>
              <li class="game-item" :key="index" v-for="(item,index) in gameData">
                <div class="game-pic">
                  <img :src="publicUrl+item.images" :class="{'mg-pic':$route.query.id==26}" alt="">
            </div>
                  <h3>{{item.name}}</h3>
                  <a v-show="Public_User=='test'||!Public_User" href="javascript:void(0)" id="try" @click="loginGame(item)">试玩游戏</a>
  ...
  methods: {
    ...
    async getData(item) {
      let res = await _SetPost(`${this.$HOST_NAME}/games/pages`, {
        ...
        tag: this.currenttype + "," + this.currentline + "," + this.currentTag,
        name: this.iptVal
      });
      if (res && res.code == 200) {
