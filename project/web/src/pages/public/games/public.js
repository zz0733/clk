...
const mixin = {  
    ...
    methods: {
        ...
        async getItemId(item){
          ...
          console.log(item)
          let routeData = this.$router.resolve({ path: '/loading', query: { name:item.name, classId: item.id,platform:item.platform } });
        ...
        trustLogin(item){
          ...
          let routeData = this.$router.resolve({ path: '/loading', query: { platform:item.platform, gameName: item.gameName,name:item.name ,login:true} });
          window.open(routeData.href,"newwindow","height=750,width=1285, top=0, left=" +(window.screen.availWidth - 1295) / 2 +"");       
        } , 
        // 需要解析再登录，如第三方电子,需要解析
        loginGame(item){
          ...
          let routeData = this.$router.resolve({ path: '/loading', query: {name:item.name, platform:JSON.parse(item.special_setting).platform, gameName: JSON.parse(item.special_setting).gameName,name:item.name ,login:true} });
          window.open(routeData.href,"newwindow","height=750,width=1285, top=0, left=" +(window.screen.availWidth - 1295) / 2 +"");            
        },                     
    },
    beforeDestroy() {
