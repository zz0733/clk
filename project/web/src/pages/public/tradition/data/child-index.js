...
import vpRanking from "@/pages/public/tradition/components/ranking/ranking";
        ...
        outItem.list.forEach((intItem) => {
          intItem.mess.forEach((thirItem) => {
            ...
            if (
              ...
              +newPrice.toString().substring(newPrice.toString().length - 2) ==
              0
            ) {
              thirItem.price = newPrice
                .toString()
                .substring(0, newPrice.toString().length - 2);
            } else if (
              newPrice.toString().includes(".") &&
              +newPrice.toString().substring(newPrice.toString().length - 1) ==
              0
            ) {
              thirItem.price = newPrice
                .toString()
                .substring(0, newPrice.toString().length - 1);
            } else {
              thirItem.price = newPrice;
            }
          })
    ...
    submit(type) {
      ...
      this.submitList.forEach((item) => {
        ...
        if (type && type.code === 'dpc-zsp') {
          submitArr.push({ "code": item.title, "playway": item.code, "odds": item.fixedOdds, "money": finalModel, "ball": item.ball, "attach": "" })
        } else {
