...
<div class="lottery-content-all-right-submit">
    <div class="decial-right-title">共 <span>{{totalCount.number}}</span> 注，总额 <span>{{totalCount.total.length>10?totalCount.total.toFixed(4):totalCount.total}}</span> 元</div>
    ...
    <div class="decial-right-select">
      <div :key="index" v-for="(item,index) in selectFast.list" @click="chooseFastMoney(item,index)" :class="{'active':item.active}">
