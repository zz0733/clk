        ...
        <div class="vp-lottery-trend-content">
            ...
            <div ref="contentWrap" style="display:none;">
                <div class="trend-toggle">
                    ...
                    <span class="issue">
                        <span :class="{'active':issueDatas.name=='最近20期'}" @click="issueFc('最近20期',20)" class="periodbtn">最近20期</span>
                        <span :class="{'active':issueDatas.name=='最近30期'}" @click="issueFc('最近30期',30)" class="periodbtn">最近30期</span>
                        <span :class="{'active':issueDatas.name=='最近50期'}" @click="issueFc('最近50期',50)" class="periodbtn">最近50期</span>
                        <span :class="{'active':issueDatas.name=='最近100期'}" @click="issueFc('最近100期',100)" class="periodbtn">最近100期</span>
                    </span>
                </div>
                        ...
                        <div class="trend-left-box">
                            <div class="trend-left-box-top">
                                ...
                                <div class="td-issue">
                                    ...
                                    <div :key="index" v-for="(item,index) in newDatas.leftDatas.newIssue" class="issue-number">{{lotteryId==2||lotteryId==10?item:item.toString().substring(4,item.length)}}</div>
                                </div>
                                <div class="td-issue">
                                    <div class="left-title">开奖时间</div>
                                    <div :key="index" v-for="(item,index) in newDatas.leftDatas.newTimeArr" class="issue-number">{{item}}</div>
                                </div>                                
                                <!-- 开奖号 -->
                                <div class="td-lottery-number">
                                    <div class="left-title">开奖号</div>
                                    <div class="td-lottery-number-item">
                                        <div :key="outIndex" v-for="(outItem,outIndex) in newDatas.leftDatas.newResult" class="lottery-number">
                                ...
                                <div class="td-lottery-wide">
                                    ...
                                    <div class="wide-wrap" :key="outIndex" v-for="(outItem,outIndex) in newDatas.leftDatas.newSingle">
                                        <div class="wide-inner-wrap">
                                            <div :class="{'active':intItem=='大'||intItem=='单'}" :key="intIndex" v-for="(intItem,intIndex) in outItem" class="item">{{intItem}}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="vp-lottery-trend-cavans" :key="outInder" v-for="(outItem,outInder) in drawDeclList">
                                    ...
                                    <div class="trend-out-box">
                                        <div class="row" :key="intInder" v-for="(intItem,intInder) in outItem.list">
                                            <div @click='drawPicFc(thirdItem,outInder,intInder,thirdIndex)' :key="thirdIndex" v-for="(thirdItem,thirdIndex) in intItem.ballList">
                                                <span>{{checkDatas[1].staue?thirdItem.lost:''}}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <canvas v-show="checkDatas[0].staue" :ref="'trendCanvas'+(outInder+1)" :key="intInder" v-for="(intItem,intInder) in outItem.list"></canvas>
                                    <div class="refBall-list">
                                      <div :class="{'active':outInder%2!=0}" :ref="'postBall'+(outInder+1)" :key="intInder" v-for="(intItem,intInder) in outItem.list">
                                        <span>
                                            {{intItem.activeBall}}
                                        </span>
                                      </div>
...
export default {
  data() {
    return {
      isLoading: false,
      titleLeft: ["万位","千位","百位","十位","个位","前三","中三","后三","和值","大小","单双","牛牛","梭哈"],
    ...
    async getHisDatas() {
      ...
      if (res && res.code == 200) {
        ...
        let rankConfig = [
          ...
          { index: 4, name: "gw" },{ index: 5, name: "qs" },{ index: 6, name: "zs" },{ index: 7, name: "hs" },{ index: 8, name: "hz" },{ index: 9, name: "dx" },
          { index: 10, name: "ds" },{ index: 11, name: "nn" },{ index: 12, name: "sh" }
        ];   
    ...
    .check {
      label {
        ...
        .check-ico {
          ...
          &.active {
            border: 0;
            background: url("/static/public/image/lottery/trend/vp-trend-true-bg.png")
              #ff5151 center no-repeat;
          }
        }
        .check-name {
          padding-left: 3px;
