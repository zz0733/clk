...
<div>
  ...
  <div class="get-money">
    ...
    <div class="withdraw">
      ...
      <van-field type="number" v-model="amount" :placeholder="'可取款金额'+availableAmount" id="moneyNum" onkeyup="value=value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')" @blur="hanlderAmount" />
    ...
    <van-number-keyboard :show="showKeyboard" class="numberKey" @input="onInput" @delete="onDelete" @blur="showKeyboard = false" />
  ...
  methods: {
    ...
    getBank() {
      if (localStorage.getItem("selectBank")) {
        this.bankInfo = JSON.parse(localStorage.getItem("selectBank"));
        this.$loading(false);
      } else {
        this.$http.post(`${this.$HOST_NAME}/member/bank`).then(res => {
          if (res.code == 200) {
            ...
            localStorage.setItem("selectBank", JSON.stringify(res.data[0]));
            this.$loading(false);
          } else {
            this.$loading(false);
          }
