<template>
  <div class="set-bank">
    ...
    <van-area v-if="areaShow" :area-list="areaList" :columns-num="2" class="van-area" @confirm="hanlderArea" @cancel="areaShow=false" />
    <van-picker v-if="bankNmaeShow" class="van-area" show-toolbar @confirm="hanlderBankName" :columns="commBank" @cancel="bankNmaeShow=false" />
    <van-picker v-if="secretShow" class="van-area" show-toolbar @confirm="hanlderSecret" :columns="secretData" @cancel="secretShow=false" />
  </div>
</template>
          ...
          if (res && res.code == 200) {
            ...
            this.$http.post(`${this.$HOST_NAME}/member/bank`).then(res => {
              if (res.code == 200) {
                ...
                localStorage.setItem("bankData", JSON.stringify(res.data));
                this.$router.go(-1);
              }
            });
