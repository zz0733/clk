...
<div class="deposit-form" v-if="$route.query.classType!='qr_code'">
  ...
  <div class="off-line" v-if="depositBank.length !==0&&$route.query.name==='银行转账'">
    <div class="course">
      ...
      <h3>选择汇款卡号</h3>
      <ul class="clearfloat">
        <li v-for="(item,i) in depositBank" :key="i" @click="toggleLi(i,item)" :class="{activeLi:activeLi===i}">
    ...
    <div class="bank-info">
      <div class="row">
        ...
        <a @click="onCopy(deoisitList.cardName)">复制</a>
      </div>
      <div class="row">
        <label>收款账号</label>
        <span>{{deoisitList.cardNum}}</span>
        <a @click="onCopy(deoisitList.cardNum)">复制</a>
      </div>
      <div class="row">
        <label>开户网点</label>
        <span>{{deoisitList.cardAddress}}</span>
        <a @click="onCopy(deoisitList.cardAddress)">复制</a>
      </div>
      <div class="row">
        <label>汇款方式</label>
    ...
    <div class="payBtn" @click="bankSubmit" :class="{'active':!canClick}">
      确认提交
    </div>
  ...
  <div class="on-line" v-else-if="categoryData.length !==0">
    <div style="height:.2rem;background:#f9f9f9;"></div>
    ...
    <div class="recharge">
      ...
      <ul class="clearfloat">
        <li v-for="(item,i) in moneyData" @click="mountMoney(item,i+1)" :key="i" :class="{mountMoneyActive:mountMoneyActive ==i+1}">{{item}}</li>
      </ul>
    </div>
    <div class="payBtn" @click="onlinePayment" :class="{'active':!canClick}">
      支&nbsp;付
    </div>
    <div class="warmPrompt">
  ...
  <div v-else class="no-pay-tongdao">
    <img src="../../../../../assets/img/public/image/common/personal/userInfo/no-pay-tongdao.png" alt="" width="60%">
  </div>
  <!-- </cube-scroll> -->
</div>
<div v-else class="saveBox deposit-form">
  ...
  <div v-if="categoryData.length !=0" class="wrap">
    ...
    <div class="inputBox">
      ...
      <div class="inpItem">
        <label>存款金额：</label>
        <cube-input v-model="passKey.money" type="tel" :placeholder="'单笔限额'+passKey.min_amount+'~'+passKey.max_amount+'元'"></cube-input>
      </div>

      <!-- <input type="tel" v-model="mount" :placeholder="`单笔消费不能超过 ${maxMoney} 元！`"> -->
    </div>
    <div class="payBtn" @click="submitCode" :class="{'active':!canClick}">
      确认提交
    </div>
    <div class="warmPrompt">
      <span>
...
export default {
  ...
  computed: {
    imgArr() {
      return JSON.parse(localStorage.getItem("paylist"));
    },
    staticsPath() {
      return (
        localStorage.getItem("config") &&
        JSON.parse(localStorage.getItem("config")).statics
      );
    }
  },
  filters: {
  ...
  methods: {
    ...
    bankSubmit() {
      ...
      if (
        ...
        JSON.parse(localStorage.getItem("config")).limit.paymentLimit
      ) {
        this.$newToast(
          "存款金额不能小于" +
          JSON.parse(localStorage.getItem("config")).limit.paymentLimit +
...
.deposit-form {
  ...
  .warmPrompt {
    ...
    line-height: 0.36rem;
    text-align: justify;
    font-family: Arial, "STXihei", "华文细黑", "Microsoft YaHei", "微软雅黑", "SimSun", "宋体", "Heiti", "黑体";
    text-indent: -0.08rem;
    span {
      line-height: 0.48rem;
    }
  ...
  /deep/ .van-tabs--line .van-tabs__wrap {
    ...
    .van-tab {
      ...
      //   height: 1.36rem;
      //   width: 1.2rem;
      //   background: url("../../../../../assets/img/public/image/common/personal/userInfo/round@3x.png")
      //     no-repeat;
    ...
    //   .tab-title {
    //     height: 1.5rem;
    //     background: url("../../../../../assets/img/public/image/common/personal/userInfo/roundhover@3x.png")
    //       no-repeat;
