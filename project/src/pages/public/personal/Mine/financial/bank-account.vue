<template>
    <div class="bank-account">
       ...
       <div v-else class="bank-info">
        <cube-scroll>
                <div class="row" v-for="(item,i) in bankData" :key="i" @click="selectBank(item)"  :style="{backgroundImage: 'url(' + item.imgUrl + ')', backgroundSize:'cover'}"  >                            
                   <div class="bank-name" >
                       <label></label>
                      <span>
                      {{item.bankName}}
  ...
  methods: {
    ...
    getBank() {
      this.$http.post(`${this.$HOST_NAME}/member/bank`).then(res => {
        if (res && res.code == 200) {
          ...
          this.bankData = JSON.parse(localStorage.getItem("bankData"));
          this.$loading(false);
        } else {
          this.$loading(false);
        }
    ...
    selectBank(bank) {
      if (this.$route.query.id == "2") {
        ...
        localStorage.setItem("selectBank", JSON.stringify(bank));
