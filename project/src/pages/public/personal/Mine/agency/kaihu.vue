...
<div class="open-account">
  ...
  <!-- 用户管理 -->
  <div class="account-user" v-if="spanActive ==0">
    <!-- 开户内容 -->
    <div class="account-content" v-if="!refundShow">
      <!-- 开户标题 -->
      <div class="title">
        <ul>
      ...
      <div class="open-user" v-if="openActive ==0">
        <cube-scroll>
          ...
          <div class="row">
            <label>返点:</label>
            <cube-select v-model="params.rebate" :options="options1"></cube-select>
          </div>
      ...
      <!-- 生成链接 -->
      <div class="open-user" v-if="openActive ==1">
        <cube-scroll>
          <div class="row">
            <label>返点:</label>
            <cube-select v-model="params.rebate" :options="options1"></cube-select>
    ...
    <div class="refundSet" v-if="refundShow">
      ...
      <cube-scroll>
        ...
        <div class="refund-content">
          ...
          <div class="refund-row" v-for="(item,i) in refund.list" :key="i">
            <label>{{item.name}}:</label>
            <cube-select v-model="item.value" :options="item.value|redundCaplitalize"></cube-select>
          </div>
        </div>
  ...
  <doiams v-if="spanActive ==1"></doiams>
...
export default {
  data() {
    return {
      ...
      params: {
        rebate: String(
          parseInt(this.$store.state.mainState.userinfo.agencyInfo.rebate)
        )
      },
      options1: [],
      refundData: [],
  ...
  methods: {
    ...
    refundInfo() {
      this.$http.post(`${this.$HOST_NAME}/member/refundInfo`).then(res => {
        if (res.code == 200) {
          ...
          localStorage.setItem("refund", JSON.stringify(this.refundData));
    ...
    // 反水统一增加
    increase() {
      let verifyrefund = JSON.parse(localStorage.getItem("refund"));
      // let maxRefund;
