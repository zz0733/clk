    ...
    <div class="all-container-wrap">
      <transition name="fade">
        <div class="all-container-betArea">
          ...
          <vp-nav class="fuCaiNav" :childNeedMess='childNeedMess' :sideReady='sideReady' :side='side' v-on:child-say="getListData"></vp-nav>
          <div class="betArea-decial-box">
            <div class="lottery-cont" ref="lottery-cont">
                <div class="wrap" v-if="sideSelect=='二字定位'">
                  ...
                  <div v-if="active.erMol==outItem.small" class="list" :key=outIndex v-for="(outItem,outIndex) in showList.list">
                    <div class="lottery-title">{{outIndex==0||outIndex==1||outIndex==2?outItem.small.substring(0,1):outItem.small.substring(1,2)}}</div>
                    <ul class="lottery-ul">
                      <li :class="{
                  ...
                  <div v-if="active.erMol==outItem.small" class="list" :key=outIndex v-for="(outItem,outIndex) in showList.list">
                    ...
                    <ul class="lottery-ul">
                      <li :class="{
                        ...
                        <div :class="{'active':intItem.state}" class="ball">{{intItem.name}}</div>
                        <div :class="{'active':intItem.state}" class="odd">{{(+intItem.odds*(1-total.rebate*0.01)).toFixed(2)}}</div>
                      </li>
                    </ul>
                  </div>
                  <div @touchmove.prevent v-show="closeMaskStaue" class="lottery-close-mask" ref="lottery-close-mask">
                  ...
                  <div class="list" :key=outIndex v-for="(outItem,outIndex) in showList.list">
                    ...
                    <ul class="lottery-ul">
                      <li :class="{
                        ...
                        <div :class="{'active':intItem.state}" class="ball">{{sideSelect=='三字组合'?intItem.code:intItem.name}}</div>
                        <div :class="{'active':intItem.state}" class="odd">{{(+intItem.odds*(1-total.rebate*0.01)).toFixed(2)}}</div>
                      </li>
                    </ul>
                  </div>
