<template>
    <div ref="vp-luzhu-wrap" class="vp-luzhu-wrap">
        <div class="header">
            <div @click="tag(ballList[0].title)" :class="{'active':this.selDatas.includes('球')||this.selDatas.includes('名')||this.selDatas.includes('军')}" class="select-wrap item">
                <div class="title">
                  <span :class="{'active':this.selDatas.includes('球')||this.selDatas.includes('名')||this.selDatas.includes('军')}">{{ball}}</span>
                </div>
            </div>
            <div @click="tag('两面长龙')" :class="{'active':selDatas=='两面长龙'}" class="el item">两面长龙</div>
        </div>
        <div class="cont" v-show="selDatas=='两面长龙'">
            <ul class="zhangLong-ul">
                <li class="zhangLong-li" v-for="(item, index) in list"  :key='index'>
                    ...
                    <div>{{Object.entries(item)[0][1]}}期</div>
                </li>
            </ul>
        </div> 
        <div class="cont"  v-show="this.selDatas.includes('球')||this.selDatas.includes('名')||this.selDatas.includes('军')">
            <div class="nav">
              <van-tabs v-model='vanActive' @click="select" class="hall-container-header luZhuTitle">
                <van-tab v-if="+ball.substring(1,2)>5&&$route.meta.series=='pk10'&&item.title=='龙虎'?0:1" :key="index" v-for="(item,index) in itemList" :title="item.title">
                </van-tab>
              </van-tabs> 
            </div>
